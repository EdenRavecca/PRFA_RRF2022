...
# load workspace 
load( "SG_HMM_Workspace.RData")

######################          Clean Data          ############################

# create reduced dataframe that only includes columns of interest
colnames( SG_df )[6] <- "Easting"
colnames( SG_df )[7] <- "Northing"

head( SG_df )
SG_df <- SG_df %>%
  dplyr::select( ID, t_, Easting, Northing, alt, scaled_alt )
...
#######################          Prep Data          ############################
#.................      Using momentuHMM::prepData     .........................


?prepData

SG_prep <- momentuHMM::prepData( SG_df, type="UTM", coordNames=c("Easting","Northing") )
head( SG_prep )
summary( SG_prep ) # Max step length is 316m ! That's awesome
class( SG_prep )
plot( SG_prep )
hist( SG_prep$alt )
hist( SG_prep$step, breaks = 100 )
hist( SG_prep$angle, breaks = 100 )
summary( SG_prep$angle )

unique( SG_prep$ID )

# standardize covariate values -- already done using function scale( alt )
# SG_data$alt <- (SG_data$alt-mean(SG_data$alt))/sd(SG_data$alt)
...
# acf for this animal is only about 3

#######################          FIT HMM          ##############################
#...........    1 individual SG using momentuHMM::fitHMM    ....................
...
m1_SG <- momentuHMM::fitHMM( data = SG_prep, nbStates = 2, 
                             dist = list( step = "gamma", angle = "vm" ),
                             Par0 = list(step = stepPar0, angle = anglePar0),
...
# view results
plot( m1_SG )
print( m1_SG ) # estimations
plotPR( m1_SG ) # checking residuals, but not sure what I'm looking at
...
Par0 <- momentuHMM::getPar0(model=m1_SG, nbStates=2,
                            DM=list(step = stepDM, angle= angleDM),
...
#####   Covariate Model m2_SG, Parameter Estimates for State Distribution  #####
#..........      Covariate Models, state distribution formula :        .........

# provide initial values for the coefficients in each state's STEP distribution
# ORDER: int_mean1, int_mean2, int_sd1,  int_sd2, int_zeromass1, int_zeromass2
stepPar0_alt <- c( 3.508092, 4.251077, 3.405863, 3.205991, -4.592400, -28.568315)

# provide initial values for the coefficients in each state's ANGLE distribution
# ORDER: int_mean1, int_mean2, int_concentration1, beta1_concentration1, int_concentration2, beta1_concentration2)
anglePar0_alt <- c(0.0023978079, 0.0001732104, -0.4206254134, 0, 2.7229227455, 0)

# required to define equations for the parameters in state distributions:
# since equations the same for each of two states, only need to list equation once:

# m2_SG model formula 2 states
# STEP ~ gamma( mu_int, sd_int, zerommass_int)
...
plot(m2_SG)
print(m2_SG) # estimations
plotPR(m2_SG)

#####   Covariate Model m3_SG, Parameter Estimates for State Distribution  #####
#..........      Covariate Models, state distribution formula :        .........
...
# provide initial values for the coefficients in each state's STEP distribution
# int_mean1, beta1_mean1, int_mean2, beta1_mean2, int_sd1,  beta1_sd1, int_sd2, beta1_sd2, int_zeromass1, int_zeromass2
stepPar0_alt_full <- c( 3.508092, 0, 4.251077, 0, 3.405863, 0, 3.205991, 0, -4.592400, -28.568315)

# provide initial values for the coefficients in each state's ANGLE distribution
# int_mean1, int_mean2, int_concentration1, beta1_concentration1, int_concentration2, beta1_concentration2
anglePar0_alt_full <- c(0.0023978079, 0.0001732104, -0.4206254134, 0, 2.7229227455, 0)
...
plot(m3_SG)
print(m3_SG) # estimations
plotPR(m3_SG)

save.image("SG_HMM_Workspace.RData")
